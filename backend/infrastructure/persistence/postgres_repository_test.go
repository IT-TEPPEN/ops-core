...
func (s *PostgresRepositoryTestSuite) createTestRepository() model.Repository {
	id := uuid.New().String()
	name := "test-repo-" + id[:8]
	url := "https://github.com/example/" + name
	accessToken := "test-token-" + id[:8]
	return model.NewRepository(id, name, url, accessToken)
}
...
func (s *PostgresRepositoryTestSuite) TestSave() {
	...
	s.T().Run("同じIDの既存リポジトリが更新される", func(t *testing.T) {
		...
		// 同じIDで別の値を持つリポジトリを作成
		updatedRepo := model.ReconstructRepository(
			repo.ID(),
			repo.Name()+"-updated",
			repo.URL()+"-updated",
