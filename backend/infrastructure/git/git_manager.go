...
type GitManager interface {
	...
	ListRepositoryFiles(ctx context.Context, localPath string) ([]string, error)
	// ValidateFilesExist checks if the given file paths exist in the repository.
	ValidateFilesExist(ctx context.Context, localPath string, filePaths []string) error
	// ReadManagedFileContent reads the content of a specific file from the local repository.
	ReadManagedFileContent(ctx context.Context, localPath string, filePath string) ([]byte, error)
}
