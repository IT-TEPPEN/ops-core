...
func TestRegister(t *testing.T) {
	...
	t.Run("有効なリポジトリURLで正常に登録できる", func(t *testing.T) {
		...
		repoMatcher := mock.MatchedBy(func(r model.Repository) bool { return true })
	...
	t.Run("既に存在するリポジトリURLでエラーになる", func(t *testing.T) {
		...
		accessToken := "test-token"
		existingRepo := model.NewRepository(uuid.NewString(), "existing-repo", url, "")
...
func TestGetRepository(t *testing.T) {
	...
	t.Run("存在するリポジトリIDで正常に取得できる", func(t *testing.T) {
		...
		expectedRepo := model.NewRepository(repoID, "test-repo", "https://github.com/example/test-repo", "test-token")
...
func TestListRepositories(t *testing.T) {
	...
	t.Run("リポジトリ一覧が正常に取得できる", func(t *testing.T) {
		...
		expectedRepos := []model.Repository{
			model.NewRepository(uuid.NewString(), "repo1", "https://github.com/example/repo1", "token1"),
			model.NewRepository(uuid.NewString(), "repo2", "https://github.com/example/repo2", "token2"),
		}
	...
	t.Run("リポジトリが1つも存在しない場合は空のスライスが返る", func(t *testing.T) {
		...
		// 空のリポジトリリスト
		var emptyRepos []model.Repository
...
func TestListFiles(t *testing.T) {
	...
	t.Run("存在するリポジトリのファイル一覧が正常に取得できる", func(t *testing.T) {
		...
		repoID := uuid.NewString()
		testRepo := model.NewRepository(repoID, "test-repo", "https://github.com/example/test-repo", "test-token")
	...
	t.Run("アクセストークンが設定されていない場合はエラーになる", func(t *testing.T) {
		...
		repoID := uuid.NewString()
		testRepo := model.NewRepository(repoID, "test-repo", "https://github.com/example/test-repo", "") // 空のアクセストークン
...
func TestUpdateAccessToken(t *testing.T) {
	...
	t.Run("アクセストークンが正常に更新される", func(t *testing.T) {
		...
		repoID := uuid.NewString()
		testRepo := model.NewRepository(repoID, "test-repo", "https://github.com/example/test-repo", "old-token")
		newToken := "new-token"
