...
func (m *MockRepositoryUseCase) Register(ctx context.Context, repoURL string, accessToken string) (model.Repository, error) {
...
func (m *MockRepositoryUseCase) GetRepository(ctx context.Context, repoID string) (model.Repository, error) {
...
func (m *MockRepositoryUseCase) ListRepositories(ctx context.Context) ([]model.Repository, error) {
	args := m.Called(ctx)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockRepositoryUseCase) ListFiles(ctx context.Context, repoID string) ([]model.FileNode, error) {
	args := m.Called(ctx, repoID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).([]model.FileNode), args.Error(1)
}
