...
func TestRegisterRepository(t *testing.T) {
	t.Run("正常に登録できる場合", func(t *testing.T) {
		...
		// モックの振る舞いを定義
		createdRepo := model.ReconstructRepository(
			uuid.NewString(),
			"test-repo",
			repoURL,
			accessToken,
...
func TestListRepositories(t *testing.T) {
	t.Run("正常にリポジトリ一覧を取得できる場合", func(t *testing.T) {
		...
		// テストデータ
		repo1 := model.NewRepository(
			uuid.NewString(),
			"repo1",
			"https://github.com/example/repo1",
			"token1",
		)
		repo2 := model.NewRepository(
			uuid.NewString(),
			"repo2",
			"https://github.com/example/repo2",
			"token2",
		)
		repos := []model.Repository{repo1, repo2}
...
func TestListRepositoryFiles(t *testing.T) {
	t.Run("正常にファイル一覧を取得できる場合", func(t *testing.T) {
		...
		fileNodes := []model.FileNode{
			model.NewFileNode("README.md", "file"),
			model.NewFileNode("src/main.go", "file"),
			model.NewFileNode("docs/index.md", "file"),
		}
...
func TestSelectRepositoryFiles(t *testing.T) {
	t.Run("正常にファイルを選択できる場合", func(t *testing.T) {
		...
		requestBody := SelectFilesRequest{
			FilePaths: filePaths,
		}
		jsonBody, _ := json.Marshal(requestBody)
		...
		var response SelectFilesResponse
		err := json.Unmarshal(rec.Body.Bytes(), &response)
		require.NoError(t, err)
...
func TestGetSelectedMarkdown(t *testing.T) {
	t.Run("正常にMarkdown内容を取得できる場合", func(t *testing.T) {
		...
		var response GetMarkdownResponse
		err := json.Unmarshal(rec.Body.Bytes(), &response)
		require.NoError(t, err)
		assert.Equal(t, repoID, response.RepoID)
...
func TestUpdateAccessToken(t *testing.T) {
	t.Run("正常にアクセストークンを更新できる場合", func(t *testing.T) {
		...
		requestBody := UpdateAccessTokenRequest{
			AccessToken: accessToken,
		}
		jsonBody, _ := json.Marshal(requestBody)
