...
func TestNotFoundError_Error(t *testing.T) {
	...
	expected := "[APP_RES_001] Repository not found (ID: repo-123)"
...
func TestUnauthorizedError_Error(t *testing.T) {
	...
	expected := "[APP_AUTH_001] unauthorized: invalid token"
	if err.Error() != expected {
		t.Errorf("Expected error message '%s', got '%s'", expected, err.Error())
	}
}
...
func TestForbiddenError_Error(t *testing.T) {
	...
	expected := "[APP_AUTH_002] user user-123 is forbidden to delete on resource Repository"
	if err.Error() != expected {
		t.Errorf("Expected error message '%s', got '%s'", expected, err.Error())
	}
}
...
func TestConflictError_Error(t *testing.T) {
	...
	expected := "[APP_RES_002] conflict: Repository with identifier 'https://github.com/test/repo' already exists: URL already registered"
	if err.Error() != expected {
		t.Errorf("Expected error message '%s', got '%s'", expected, err.Error())
	}
}
...
func TestValidationFailedError_Error(t *testing.T) {
	...
	expected := "[APP_VAL_001] validation failed: 2 error(s)"
	if err.Error() != expected {
		t.Errorf("Expected error message '%s', got '%s'", expected, err.Error())
	}
}
