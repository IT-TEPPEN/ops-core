...
func TestExternalAPIError_Error(t *testing.T) {
	...
	expected := "[INF_EXT_001] external API error calling GitHub at /repos/owner/repo (status 500): internal server error"
...
func TestConnectionError_Error(t *testing.T) {
	...
	expected := "[INF_CONN_001] connection error to database: connection refused"
	if err.Error() != expected {
		t.Errorf("Expected error message '%s', got '%s'", expected, err.Error())
	}
}
...
func TestStorageError_Error(t *testing.T) {
	...
	expected := "[INF_STOR_001] storage error during Upload at path /tmp/file.txt: disk full"
	if err.Error() != expected {
		t.Errorf("Expected error message '%s', got '%s'", expected, err.Error())
	}
}
