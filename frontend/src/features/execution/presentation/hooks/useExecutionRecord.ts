...
import type { ExecutionRecordViewData } from "../../application";
import {
...
/**
 * Hook for managing execution records.
 * Refactored to use ExecutionQueryService and ExecutionCommandService following ADR 0019.
 */
export function useExecutionRecord({
  ...
  const queryService = useExecutionQueryService();
  const commandService = useExecutionCommandService();
  ...
  useEffect(() => {
    ...
    const fetchRecord = async () => {
      try {
        const record = await queryService.getById(recordId);
  ...
  const handleStartExecution = async () => {
    ...
    try {
      ...
      const record = await commandService.create({
        documentId: docId,
  ...
  const handleUpdateTitle = async (newTitle: string) => {
    ...
    setIsSaving(true);
    try {
      const updated = await commandService.updateTitle(
        executionRecord.id,
        newTitle
      );
  ...
  const handleUpdateNotes = async (notes: string) => {
    ...
    setIsSaving(true);
    try {
      const updated = await commandService.updateNotes(
        executionRecord.id,
        notes
      );
      setExecutionRecord(updated);
  ...
  const handleAddStep = async (stepNumber: number, description: string) => {
    ...
    const updated = await commandService.addStep(executionRecord.id, {
      stepNumber,
      description,
    });
    setExecutionRecord(updated);
  ...
  const handleUpdateStepNotes = async (stepNumber: number, notes: string) => {
    ...
    const updated = await commandService.updateStepNotes(executionRecord.id, {
      stepNumber,
      notes,
    });
    setExecutionRecord(updated);
  ...
  const handleComplete = async () => {
    ...
    setIsSaving(true);
    try {
      const updated = await commandService.complete(executionRecord.id);
      setExecutionRecord(updated);
    } catch (err) {
  ...
  const handleFail = async () => {
    ...
    setIsSaving(true);
    try {
      const updated = await commandService.fail(executionRecord.id);
      setExecutionRecord(updated);
    } catch (err) {
